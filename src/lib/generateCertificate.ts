...
export async function generateCertificate(data: CertificateData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.fontSize(12)
         .font('Helvetica')
         .text(data.instructorName, 150, 470);

      // Certificate ID
      doc.fontSize(10)
         .font('Helvetica')
         .fillColor('#9ca3af')
         .text(`Certificate ID: ${data.certificateId}`, 0, doc.page.height - 80, { align: 'center' });

      // Platform name
      doc.fontSize(12)
         .font('Helvetica-Bold')
         .fillColor('#2563eb')
         .text('Massive Income Course Platform', 0, doc.page.height - 60, { align: 'center' });
