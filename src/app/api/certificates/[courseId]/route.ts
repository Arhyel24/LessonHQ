  ...
  try {
      ...
      try {
        ...
        return new NextResponse(certificateBuffer, {
          headers: {
            ...
            'Content-Disposition': `attachment; filename="certificate-${course.title.replace(/[^a-zA-Z0-9]/g, '-')}.pdf"`,
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        certificateId: `${user._id}-${course._id}`,
        downloadUrl: `/api/certificates/${params.courseId}?download=true`
      }
    });
  ...
  try {
    ...
    if (!progress) {
      ...
      progress = new Progress({
        user: userId,
        course: params.courseId,
        lessonsCompleted: Array.from({ length: course.lessons.length }, (_, i) => i),
        percentage: 100,
    ...
    if (forceIssue) {
      ...
      progress.lessonsCompleted = Array.from({ length: course.lessons.length }, (_, i) => i);
    }
